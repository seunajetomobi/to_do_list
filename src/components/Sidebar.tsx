...
export default function Sidebar({ lists, selectedId, onSelect, onNew, onRename, onDelete }: SidebarProps) {
  ...
  return (
    <aside className="sidebar">
      ...
      <div className="lists-section">
        {lists.length === 0 && (
          <div style={{ padding: '16px 20px', color: 'rgba(199,210,254,0.4)', fontSize: '0.82rem', textAlign: 'center' }}>
        ...
        {lists.map((list) => (
          ...
          >
            {editingId === list.id ? (
              ...
              <span className="list-item-name" title={list.name}>{list.name}</span>
            )}
